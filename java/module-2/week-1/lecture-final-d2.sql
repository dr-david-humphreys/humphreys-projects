...
SELECT price, pizza_id, size_id, crust, sauce
FROM pizza
WHERE size_id = 'S'
...
FROM sale
WHERE total 
BETWEEN 10.49 AND 15.99 --Inclusive
ORDER BY total;
...
SELECT COUNT(phone_number) AS phone_number_count FROM customer; 
...
--**D2** HAVING (must be used with/after a GROUP BY)
SELECT topping_name, COUNT(*) AS times_used
FROM pizza_topping
GROUP BY topping_name
HAVING COUNT(*) > 5;   
