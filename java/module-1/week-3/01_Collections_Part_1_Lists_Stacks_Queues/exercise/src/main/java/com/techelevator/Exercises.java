...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		for (String array2List : stringArray) {
			newList.add(array2List);
		}
		return newList;
	}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for (int num : intArray) {
			listOfDoubles.add((double) num / 2);
		}
	...
	** INTERVIEW QUESTION **
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for (int num : integerArray) {
			if (num % 3 == 0 && num % 5 == 0) {
				...
				listOfFizzBuzz.add(Integer.toString(num));
			}
		}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		List<Integer> newListInterwoven = new ArrayList<>();
		int firstList = listOne.size();
		int secondList = listTwo.size();
		int leftover = Math.max(firstList, secondList);
		for (int i = 0; i < leftover; i++) {
			if (i < firstList) {
				newListInterwoven.add(listOne.get(i));
			}
			if (i < secondList) {
				newListInterwoven.add(listTwo.get(i));
			}
		}
		return newListInterwoven;
	}
